...
var handleRequest = async (request, env) => {
 ...
 // Extract the arc claim from the JWT token payload (without validation) only if present
 let arcClaim;
 try {
   const decoded = index_default.decode(approovToken);
   arcClaim = decoded && decoded.payload ? decoded.payload.arc : undefined;
 } catch (e) {
    // default setting meaning the token does not contain "arc" claim
   arcClaim = "unknown-arc-claim";
 }
 // arcClaim is now available for later use
 ...
 if (ctx.approovBindingVerification) {
   ...
   const isAuthorized = await validateBinding(ctx, approovToken, approovBinding);
   if (!isAuthorized) {
